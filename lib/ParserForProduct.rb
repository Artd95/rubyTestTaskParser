...
class ProductPagesParser
  ...
  def parse_product
    ...
    # else
    doc.xpath("//div[contains(@class,'attribute_list')]/ul/li/label/span[contains(@class,'radio_label')]/text()").each_with_index do |product,i|  #The loop iterates multi-products
      @product_name = "#{doc.xpath("//h1[contains(@class, 'product_main_name')]/text()")} - #{doc.xpath("//div[contains(@class,'attribute_list')]/ul/li[#{i+1}]/label/span[contains(@class,'radio_label')]/text()")}"
      @product_price = doc.xpath("//div[contains(@class,'attribute_list')]/ul/li[#{i+1}]/label/span[2]/text()")
      @image_link = doc.xpath("//img[contains(@id,'bigpic')]/@src")
      $parsed_products.push(Product.new([@product_name, @product_price, @image_link]))              # Save product to array
    end
    # end
  end
